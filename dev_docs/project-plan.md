...
| DMN model discovery in pod-split deployment | ✅ Complete | acp-writer | In the monolith, `_dynamic_models` in `api.py` tracks deployed DMN models in-memory. In the pod-split deployment, DMN XML is delivered to the decision-engine pod but the guideline resolver in the llm-reasoning pod cannot discover them — `applicable_dmn_models` is always empty. Fix: either have the decision-engine pod expose a model listing endpoint that the guideline resolver queries, or store deployed model summaries in MinIO/a shared registry so both pods can see them. |
| Guideline Resolver can't see in-process models (single-pod) | Won't fix | acp-writer | `guideline_resolver.py:114` always fetches models over HTTP from `DECISION_ENGINE_URL` while `dmn_executor.py`/api use the in-process `_dynamic_models` dict. In the default single-pod deploy that host doesn't resolve, so `all_models = []` and no DMN is ever executed. Compounds the pod-split "DMN model discovery" item above, but also breaks the monolith. Single-pod mode is no longer a supported deployment. |
