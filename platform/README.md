...
|---|---|---|
| `litellm/` | LiteLLM inference gateway — local stand-in for MaaS, routing to OpenAI | Phase 1 |
...
LiteLLM provides a unified OpenAI-compatible endpoint that proxies requests to OpenAI GPT-5.6. In Phase 2, MaaS replaces LiteLLM on OpenShift — application code should not need to change since both expose the same OpenAI-compatible API.
