...
The analyst also emits a one-sentence, clinically conservative **suggested resolution** per conflict (advisory only — never auto-applied). It is shown to the clinician under the conflict in the review UI, and on a **request-changes** loop it is fed back — alongside the clinician's comment and the rest of the prior conflicts (`render_conflicts_feedback`) — so a comment like *"resolve all identified conflicts as you suggested"* has a referent. The plan is then **recomposed** (goals/activities live in the brief, so re-composing is what revises them) and the analyst re-runs: a genuinely resolved conflict simply is not re-detected. Per-conflict *structured* feedback (target one conflict, record an explicit resolution note) is deferred to [#172](https://github.com/samschifman/cpg-to-acp/issues/172). See [`docs/ai-transparency.md`](../docs/ai-transparency.md#conflict-resolution-feedback-loop) for the loop diagram.
